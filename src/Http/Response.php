...
class Response implements ResponseInterface, ArrayAccess
{
  ...
  private function getDataFromResponse(ResponseInterface $response)
  {
      ...
      if (empty($contents))
      {
        // no contents, stop processing
        return null;
      }

      if ( in_array($type, ['application/json','application/javascript']) )
      {
        //json
        return json_decode( (string) $contents ?: '{}' );
      }

      // make no assumptions, return what we got back un-parsed.
      return $contents;
  }
